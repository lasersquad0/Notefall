...
public:
	GdiPlusInit()
	{
		...
		Gdiplus::GdiplusStartup(&m_token, &startupInput, nullptr); // NOTE: For brevity I omitted error handling, check function return value!
	}
	...
	_In_ int       nCmdShow)
{
...

	{
		...
		catch (std::exception& ex)
		{
			...
			MessageBoxA(nullptr, ex.what(), "Exception caught", MB_OK | MB_ICONSTOP);
		}
		catch (...)
		{
			LOG_ERROR(MAKE_LOG_STR("UNKNOWN ERROR"));
			MessageBoxA(nullptr, "UNKNOWN ERROR", "Exception caught", MB_OK | MB_ICONSTOP);
		}
	}
...
/*
	...
	//g_hWnd` = CreateWindowEx(ES_EX_ZOOMABLE, WC_EDIT, _T("NotefallEditBox"),
...
void SearchFor(HWND hWnd, FINDREPLACE* fr)
{
	...
	size_t len2 = 0;
	if (FAILED(StringCchLength(fr->lpstrFindWhat, FIND_TEXT_LEN, &len2))) return;
...
LRESULT CALLBACK WndProcMain(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	//if (SUCCEEDED(hr))
	//{
		lRet = CaptionWndProc(hWnd, message, wParam, lParam, &fCallDWP);
	//}
