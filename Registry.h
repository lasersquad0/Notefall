...
//{
	namespace Registry
	{
		...
		public:
			...
			template <typename __Function>
			void EnumerateValues(const __Function& callback)
			{
				std::exception_ptr pex;
				...
				while(true)
				{
					...
					(
						...
						i,				// Value index
						pszName,		// Subkey name buffer
						&dwNameLen,	    // Subkey name string length
						nullptr,		// Reserved
						nullptr,		// Type of value
						nullptr,		// Buffer to receive data
						&dwDataLen		// Size of the buffer to receive data
					);
					...
					{
						auto ec = std::error_code(lStatus, std::system_category());
						//pex = std::make_exception_ptr(std::system_error(ec, "RegEnumValue() failed"));
						throw std::system_error(ec, "RegEnumValue() failed");
					}

					std::wstring subKeyName(pszName, dwNameLen);

					if (!callback(subKeyName, i))
					{
						// Break loop when callback returns false
						break;
					}
